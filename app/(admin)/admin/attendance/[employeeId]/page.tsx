...
const formatDate = (s: string) =>
  new Date(s + 'T12:00:00Z').toLocaleDateString('en-PK', {
...
export default function AdminEmployeeHistoryPage() {
  ...
  const empId    = params?.employeeId as string; // MongoDB _id
  ...
  const fetchRecords = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const res  = await fetch(`/api/admin/attendance/history?${params}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
...
const visibleRecords = selectedMonthIsCurrentMonth
  ? records.filter(r => r.date <= pkToday)
  : records;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          {empLoading ? (
            <div className="h-12 w-48 bg-slate-800 animate-pulse rounded-xl" />
          ) : employee ? (
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                    employee.isActive ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'
                  }`}>
                    {employee.isActive ? '● Active' : '● Inactive'}
                  </span>
                </div>
                <p className="text-slate-400 text-sm">
                  <code className="text-blue-400/80">{employee.employeeId}</code>
        ...
        {!loading && (
          <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl px-4 py-2 text-center">
            <p className="text-blue-400 text-xl font-bold">{totalHours.toFixed(1)}h</p>
      ...
      {!loading && (
        <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
          <StatCard label="Present"  count={stats.present}  color="text-green-400"  bg="bg-green-500/10"  border="border-green-500/20" />
          <StatCard label="Late"     count={stats.late}     color="text-yellow-400" bg="bg-yellow-500/10" border="border-yellow-500/20" />
          <StatCard label="Absent"   count={stats.absent}   color="text-red-400"    bg="bg-red-500/10"    border="border-red-500/20" />
          <StatCard label="Half Day" count={stats.halfDay}  color="text-orange-400" bg="bg-orange-500/10" border="border-orange-500/20" />
          <StatCard label="Off"      count={stats.off}      color="text-slate-400"  bg="bg-slate-500/10"  border="border-slate-600/30" />
        </div>
      )}
              ...
              <table className="w-full">
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-5 py-4">
                          <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-xs rounded-full font-medium border ${s.badge}`}>
                            <span className={`w-1.5 h-1.5 rounded-full ${s.dot}`} />
                            {statusLabel(rec.status)}
                          </span>
                        </td>
                        <td className="px-5 py-4 text-slate-400 text-xs max-w-[120px] truncate">
                          {rec.location || '—'}
                        </td>
                      </tr>
                    );
                ...
                <tfoot>
                  <tr className="border-t border-slate-700 bg-slate-800/40">
                    ...
                    <td className="px-5 py-3 font-mono text-blue-400 text-sm font-bold">
            ...
            <div className="md:hidden divide-y divide-slate-800">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <span className={`inline-flex items-center gap-1 px-2 py-0.5 text-xs rounded-full font-medium border ${s.badge}`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${s.dot}`} />
                        {statusLabel(rec.status)}
                      </span>
                    </div>
              ...
              <div className="p-4 bg-slate-800/40 flex items-center justify-between">
                <p className="text-slate-400 text-xs">
                  {stats.present + stats.late} attended · {stats.absent} absent
                </p>
      ...
      {pagination.totalPages > 1 && (
        <div className="flex items-center justify-between pt-2">
          ...
          <div className="flex gap-2">
            ...
            <span className="px-3 py-1.5 text-xs rounded-lg bg-blue-600 text-white">{pagination.page}</span>
            <button
              onClick={() => fetchRecords(pagination.page + 1)}
