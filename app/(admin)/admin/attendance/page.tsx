...
function SummaryCards({ records }: { records: AttendanceRecord[] }) {
  ...
  const cards = [
    { label: 'Present',  count: counts.present,      color: 'text-emerald-400', bg: 'bg-emerald-500/8',  border: 'border-emerald-500/15' },
    { label: 'Late',     count: counts.late,          color: 'text-yellow-400',  bg: 'bg-yellow-500/8',   border: 'border-yellow-500/15' },
    { label: 'Absent',   count: counts.absent,        color: 'text-red-400',     bg: 'bg-red-500/8',      border: 'border-red-500/15' },
    { label: 'Half Day', count: counts['half-day'],   color: 'text-orange-400',  bg: 'bg-orange-500/8',   border: 'border-orange-500/15' },
  ...
  return (
    <div className="space-y-5">
      <style>{`
        ...
        .mode-btn { transition: all 0.2s cubic-bezier(0.4,0,0.2,1); }
      ...
      <div className="flex flex-wrap gap-2.5 anim-in" style={{ animationDelay: '0.05s' }}>
        {viewMode === 'date' ? (
          <input type="date" value={dateFilter} onChange={e => setDateFilter(e.target.value)}
            className="date-input bg-white/3 border border-white/7 hover:border-white/12 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-blue-500/40 focus:ring-1 focus:ring-blue-500/20 text-sm transition-all" />
        ) : (
          <input type="month" value={monthFilter} onChange={e => setMonthFilter(e.target.value)}
            className="date-input bg-white/3 border border-white/7 hover:border-white/12 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-blue-500/40 focus:ring-1 focus:ring-blue-500/20 text-sm transition-all" />
        )}
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
          className="bg-white/3 border border-white/7 hover:border-white/12 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-blue-500/40 text-sm appearance-none transition-all">
        ...
        <div className="relative flex-1 min-w-[180px]">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search name, ID, department..."
            className="w-full bg-white/3 border border-white/7 hover:border-white/12 rounded-xl pl-10 pr-4 py-2.5 text-white placeholder-slate-600 focus:outline-none focus:border-blue-500/40 focus:ring-1 focus:ring-blue-500/20 text-sm transition-all" />
        </div>
      </div>
          ...
          <>
                ...
                <thead>
                  <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', background: 'rgba(255,255,255,0.015)' }}>
                    {['Employee', 'Date', 'Check In', 'Check Out', 'Hours', 'Status', ''].map(h => (
                      <th key={h} className="px-5 py-3 text-left text-[10px] font-bold text-slate-600 uppercase tracking-wider">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {filtered.map((rec, idx) => {
                    ...
                    return (
                      <tr key={rec._id || idx} className={`table-row ${rec.status === 'off' ? 'opacity-40' : ''}`} style={{ borderBottom: idx < filtered.length - 1 ? '1px solid rgba(255,255,255,0.03)' : 'none' }}>
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-2.5">
                            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center flex-shrink-0 shadow-sm shadow-blue-500/20">
                        ...
                        <td className="px-5 py-3.5">
                          ...
                          <p className="text-slate-600 text-[11px]">{formatWeekday(rec.date)}</p>
                        </td>
                        ...
                        <td className="px-5 py-3.5">
                          <button
                            ...
                            className="p-1.5 rounded-lg text-slate-600 hover:text-violet-400 hover:bg-violet-500/8 transition-all"
                            title="View History"
                          >
            ...
            <div className="md:hidden">
              {filtered.map((rec, idx) => {
                const s = STATUS[rec.status] || STATUS.off;
                return (
                  <div key={rec._id || idx} className={`p-4 ${rec.status === 'off' ? 'opacity-40' : ''}`} style={{ borderBottom: idx < filtered.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <p className="text-white text-sm font-medium">{rec.employee?.name ?? '—'}</p>
                          <p className="text-slate-600 text-[11px]"><code className="text-blue-400/70">{rec.employee?.employeeId}</code></p>
                        </div>
                      </div>
                    ...
                    <button onClick={() => router.push(`/admin/attendance/${rec.employeeId}`)}
                      className="w-full text-center bg-violet-500/8 border border-violet-500/15 text-violet-400 py-2 rounded-xl text-xs font-medium hover:bg-violet-500/15 transition-all">
                      View History
                    </button>
                  </div>
                );
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-1.5">
            <button onClick={() => fetchRecords(pagination.page - 1)} disabled={pagination.page <= 1}
              className="px-3 py-1.5 text-xs rounded-lg bg-white/4 border border-white/6 text-slate-300 disabled:opacity-30 hover:bg-white/7 transition-colors">Prev</button>
            ...
            <button onClick={() => fetchRecords(pagination.page + 1)} disabled={pagination.page >= pagination.totalPages}
              className="px-3 py-1.5 text-xs rounded-lg bg-white/4 border border-white/6 text-slate-300 disabled:opacity-30 hover:bg-white/7 transition-colors">Next</button>
          </div>
        </div>
      )}
    </div>
